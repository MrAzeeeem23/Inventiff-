...
export default function IndustryCard({ industry }) {
  ...
  // Guard: if industry is undefined or null
  if (!industry) {
    return (
      <div className="bg-red-100 text-red-600 p-4 rounded-md shadow">
        ⚠️ Invalid industry data
      </div>
    );
  }

  const { name, short_description, key_focus_areas, slug } = industry;

  return (
    <div
      className="bg-white dark:bg-black/40 backdrop-blur-md rounded-2xl shadow-md p-6 hover:shadow-lg transition-all hover:scale-105"
      onMouseEnter={() => setIsHover(true)}
      onMouseLeave={() => setIsHover(false)}
    >
      <div className="flex justify-between">
        <h3 className="text-xl font-afacad font-bold dark:text-white text-gray-900 mb-2">
          {name || "Unnamed Industry"}
        </h3>
        <Building2 />
      </div>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-2">
          ...
          <span>{key_focus_areas?.length || 0} areas</span>
        </div>
        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
          <div
            ...
            style={{
              width: `${Math.min(((key_focus_areas?.length || 0) / 8) * 100, 100)}%`,
            }}
          />
        </div>
      </div>
